DatePickerControl.defaultFormat   = "Y-m-d";
...
DatePickerControl.editIdPrefix    = "cal_";          // The prefix for edit's id
DatePickerControl.displayed       = false;           // Is the calendar layer displayed?
DatePickerControl.HIDE_TIMEOUT    = 300;             // Time in ms for hide the calendar layer
...
DatePickerControl.hideCauseBlur   = true;           // Was the calendar close by onblur event?
...
DatePickerControl.init = function()
{
	...
	if (!document.getElementById("CalendarPickerControl"))
	{
		...
		if (this.calContainer.addEventListener){
			this.calContainer.addEventListener("click", cal_onContainerClick, false);
			window.addEventListener("resize", cal_onWindowResize, false);
		}
		else if (this.calContainer.attachEvent){
			this.calContainer.attachEvent("onclick", cal_onContainerClick);
			window.attachEvent("onresize", cal_onWindowResize);
		}
	}
}
...
{
	var obj = document.getElementById("cal_DEFAULT_FORMAT");
	if (obj) this.defaultFormat = obj.value;

	obj = document.getElementById("cal_SUBMIT_FORMAT");
	if (obj) this.submitFormat = obj.value;
	
	obj = document.getElementById("cal_FIRST_WEEK_DAY");
	if (obj) this.firstWeekDay = (obj.value < 0 || obj.value > 6) ? 0 : parseInt(obj.value);
	
	obj = document.getElementById("cal_WEEKEND_DAYS");
	if (obj) eval("this.weekend = " + obj.value);
	
	obj = document.getElementById("cal_AUTO_SHOW");
	if (obj) this.autoShow = obj.value == "true";
	
	obj = document.getElementById("cal_DEFAULT_TODAY");
	if (obj) this.defaultTodaySel = obj.value == "true";
	
	obj = document.getElementById("cal_CALENDAR_OFFSET_X");
	if (obj) this.offsetX = parseInt(obj.value);
	
	obj = document.getElementById("cal_CALENDAR_OFFSET_Y");
	if (obj) this.offsetY = parseInt(obj.value);
	
	obj = document.getElementById("cal_TODAY_TEXT");
	if (obj) this.todayText = obj.value;
	
	obj = document.getElementById("cal_BUTTON_TITLE");
	if (obj) this.buttonTitle = obj.value;
	
	obj = document.getElementById("cal_BUTTON_POSITION");
	if (obj) this.buttonPosition = obj.value;
	
	obj = document.getElementById("cal_BUTTON_OFFSET_X");
	if (obj) this.buttonOffsetX = parseInt(obj.value);
	
	obj = document.getElementById("cal_BUTTON_OFFSET_Y");
	if (obj) this.buttonOffsetY = parseInt(obj.value);
	
	obj = document.getElementById("cal_WEEK_NUMBER");
	if (obj) this.weekNumber = obj.value == "true";
	
	obj = document.getElementById("cal_MONTH_NAMES");
	if (obj) eval("this.Months = " + obj.value);
	
	obj = document.getElementById("cal_DAY_NAMES");
...
//	var format = ctrl.getAttribute("datepicker_format");
	var format = document.getElementById("zig_calendar_" + ctrl.id).getAttribute("datepicker_format") ;
...
//	var format = document.getElementById("zig_calendar_" + edit.id).getAttribute("datepicker_format") ;
