...
function ResizeableTextbox(field_id)
{
  var MINWIDTH = 38 ;
  var MINHEIGHT = 15 ;
...
/*  var EDGEDIFFSIZE = 2*EDGE_THICKNESS + 3;*/
  ...
  var _textArea = document.getElementById(field_id);
...
/*  _container.appendChild(_textArea);*/
	...
	var _rightHandleDrag = new dragObject(_rightEdge, null, new Position(0, 0), new Position(0, 0), moveStart, rightHandleMove, moveEnd, true);

	alert("here before update1") ;
  ...
  {
   if(element == _rightEdge)
      document.body.style.cursor = 'e-resize';
  }
  
  function moveEnd(element)
  {
	alert("here before update2") ;
    UpdatePositions();
  ...
  {
	alert("here before update7") ;
	alert("height1=" + _height) ;
    if(_width < _minWidth)
      _width = _minWidth;
    if(_width > _maxWidth)
      _width = _maxWidth;
...
/*    _textArea.style.width = (_width - TEXTDIFF) + 'px';
    ...
    _textArea.style.width = _width + 'px';
    _textArea.style.height = _height + 'px';
	_rightEdge.style.left = EDGEDIFFPOS + 'px';
    _rightEdge.style.height = EDGEDIFFSIZE + 'px';
/*    _rightHandle.style.top = ((_height - MINHEIGHT) / 2) + 'px';*/
  }
...
function ResizeableTextbox2(field_id)
{
  var MINSIZE = 38;
  var EDGE_THICKNESS = 7;
  var EDGEDIFFSIZE = 2*EDGE_THICKNESS + 3;
...
/*    _rightEdge.style.left = (_width - EDGEDIFFPOS) + 'px';
    ...
    _cornerHandle.style.left = (_width - EDGEDIFFPOS) + 'px' ;
    _cornerHandle.style.top = (_width - EDGEDIFFSIZE) + 'px' ;
...
function resize_listen(div_id,field_id,type)
{
	if(type=="input")
	{
		var rt = new ResizeableTextbox(field_id) ;
	}
	else if(type=="textarea")
	{
		var rt = new ResizeableTextbox2(field_id) ;
	}
	document.getElementById(div_id).appendChild(rt.GetContainer()) ;
	rt.StartListening();
}

function resize_initialize(div_ids,field_ids,type)
{
	var splitted_div_ids = div_ids.split(",") ;
	var splitted_field_ids = field_ids.split(",") ;
	var counter = 0 ;
	var array_length = splitted_field_ids.length ;
	while(array_length>counter)
	{
		resize_listen(splitted_div_ids[counter],splitted_field_ids[counter],type) ;
		counter++ ;
	}
}